...
class MapsFragment : Fragment(), OnRouteResultListener {
    ...
    private fun initMapControl() {
        context?.let { _context ->
            //TODO: MapsIndoors.getSolution()?.config?.enableClustering = false
            val config = MPMapConfig.Builder(requireActivity(), mMap, mMapView!!, getString(R.string.mapbox_access_token), true).build()
            MapControl.create(config) { mapControl, miError ->
                if (mapControl != null && miError == null) {
                    ...
                    building?.let {
                        mMap.setCamera(mMap.cameraForCoordinateBounds(CoordinateBoundsConverter.toCoordinateBounds(it.bounds!!)))
                    }
    ...
    private fun setupBottomSheet() {
        ...
        parentFragmentManager.beginTransaction().replace(R.id.standardBottomSheet,
    ...
    fun setRoutingView(location: MPLocation) {
        ...
        parentFragmentManager.beginTransaction().replace(R.id.standardBottomSheet,
            RoutingFragment.newInstance(location, this)
        ).commit()
    }
