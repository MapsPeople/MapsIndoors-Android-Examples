...
class DirectionsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewPager.registerOnPageChangeCallback(object : OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                mMapsFragment?.getDirectionsRenderer()?.selectLegIndex(position)
            }
        })
    ...
    fun getStepName(startStep: MPRouteStep, endStep: MPRouteStep): String {
        ...
        result = if (TextUtils.isEmpty(endStepEndFloorName)) {
            String.format("Level %s to %s", if (TextUtils.isEmpty(startStepStartFloorName)) startStepStartPointZIndex else startStepStartFloorName, endStep.endPoint.floorIndex)
        } else {
    ...
    companion object {
        @JvmStatic
        fun newInstance(route: MPRoute, location: MPLocation, mapsFragment: MapsFragment) = DirectionsFragment().apply {
            mRoute = route
            mLocation = location
    ...
    inner class DirectionsStepAdapter(fragment: Fragment) : FragmentStateAdapter(fragment) {
        ...
        override fun createFragment(position: Int): Fragment {
            if (position == mRoute?.legs?.size!! - 1) {
                return DirectionStepFragment.newInstance(
                    "Walk to " + mLocation?.name,
                    "pin",
                    mRoute?.legs!![position]?.distance?.toInt(),
                    mRoute?.legs!![position]?.duration?.toInt()
                )
            } else {
                ...
                return if (firstBuilding != null && lastBuilding != null) {
                    DirectionStepFragment.newInstance(
                        getStepName(lastFirstStep, lastStep),
                        lastFirstStep.highway,
                        leg.distance.toInt(),
                        leg.duration.toInt()
                    )
                }else if (firstBuilding != null) {
                    DirectionStepFragment.newInstance(
                        "Exit: " + firstBuilding.name,
                        "exit",
                        leg.distance.toInt(),
                        leg.duration.toInt()
                    )
